    ...
    <el-form v-else ref="formRef" :model="form" :rules="rules" label-position="top">
      ...
      <section class="profile-section">
        ...
        <article class="media-card">
          ...
          <div class="media-actions">
            ...
            <request-button v-else-if="hasBotPhoto" type="danger" plain :loading="removingBotPhoto" :disabled="busy || !capabilities.photo.remove" @click="removeBotPhoto">移除</request-button>
          </div>
        ...
        <article class="media-card">
          ...
          <div class="media-actions">
            ...
            <request-button v-else-if="hasDescriptionPhoto" type="danger" plain :loading="removingDescriptionPhoto" :disabled="busy || !capabilities.descriptionPhoto.remove" @click="removeDescriptionPhoto">移除</request-button>
          </div>
      ...
      <el-collapse v-model="expandedSections" class="advanced-sections">
        <el-collapse-item name="commands">
            ...
            <article v-for="(item, index) in form.commands" :key="item._key" class="command-card">
              <div class="command-card-heading">
                ...
                <div class="command-actions">
                  <request-button text circle :icon="ArrowUp" :aria-label="`上移第 ${index + 1} 条命令`" :disabled="busy || !capabilities.commands.write || index === 0" @click="moveCommand(index, -1)" />
                  <request-button text circle :icon="ArrowDown" :aria-label="`下移第 ${index + 1} 条命令`" :disabled="busy || !capabilities.commands.write || index === form.commands.length - 1" @click="moveCommand(index, 1)" />
                  <request-button text circle type="danger" :icon="Delete" :aria-label="`删除第 ${index + 1} 条命令`" :disabled="busy || !capabilities.commands.write" @click="removeCommand(index)" />
                </div>
              </div>
          ...
          <request-button class="full-button" :icon="Plus" :disabled="busy || !capabilities.commands.write || form.commands.length >= 100" @click="addCommand">添加命令</request-button>
        ...
        <el-collapse-item name="privacy">
          ...
          <request-button v-if="form.privacy_policy_url" class="full-button" type="danger" plain :loading="removingPrivacyPolicy" :disabled="busy || !capabilities.privacyPolicy.remove" @click="clearPrivacyPolicy">移除隐私政策</request-button>
        </el-collapse-item>
      </el-collapse>
